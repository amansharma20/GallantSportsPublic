...
import { View, StyleSheet, Dimensions, Image, TouchableOpacity, Text, ScrollView, FlatList, Linking } from 'react-native';
...
export default function ExpolreArenaDetailsScreen(props) {
    ...
    const arenaDetails = props.route.params.expolreArenaDetails
    const lat = arenaDetails.item.Latitude
    const lng = arenaDetails.item.Longitude

    const { data: data2 } = useQuery(GQLQuery.GET_ACTIVITY_BY_ARENA_ID, {
        variables: {
            ArenaId: arenaDetails.item.Id,
        },
    });
        ...
        <View style={{ flex: 1 }}>
            <ScrollView
                ...
                <View>
                    ...
                    <View>
                        <Image source={{ uri: applicationProperties.imageUrl + arenaDetails.item.ArenaImageStoragePath }} style={{ width: '100%', height: 200 }} />
                    </View>
                    {/* <Carousel
                        active
                        data={IMAGEDATA}
                        ...
                        <View style={styles.mainDetailsContainer}>
                            <View style={styles.headerTopContainer}>
                                <Text style={styles.titleText}>
                                    {arenaDetails.item.Name}
                                </Text>
                                <View style={styles.ratingContainer}>
                                    <Text style={styles.ratingText}>
                                        {arenaDetails.item.Rating}
                                    </Text>
            ...
            <View style={styles.buttonContainer}>
                <CommonButton onPress={() => navigation.navigate('ExploreArenaBookingScreen',
                    {
